  ...
  const getBillPaidList = async () => {
    ...
    try {
      const { data } = await axios.get(
        `https://dentalhouse-accountant.vimubds5.a2hosted.com/api/v2/accountant/paidBillLIst/${branch}`,
        {
          //   headers: {
  ...
  const updateBillforPaid = async (id) => {
    ...
    try {
      const res = await axios.put(
        `https://dentalhouse-accountant.vimubds5.a2hosted.com/api/v2/accountant/ChangeStatusToPaidPatientBill/${id}/${branch}`,
        billUpdateForm,
        {
          headers: {
            "Content-Type": "application/json",
