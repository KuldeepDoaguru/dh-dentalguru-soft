...
const AdminPatientProfile = () => {
  ...
  const patientProfileData = async () => {
    try {
      const { data } = await axios.get(
        `https://dentalhouse-admin.vimubds5.a2hosted.com//api/v1/admin/getPatientDataByBranchAndId/${pid}`,
        {
          headers: {
  ...
  const getOngoingTreat = async () => {
    try {
      const { data } = await axios.get(
        `https://dentalhouse-admin.vimubds5.a2hosted.com//api/v1/admin/getPatientBillByBranchAndId/${user.branch_name}/${pid}`,
        {
          headers: {
            "Content-Type": "multipart/form-data",
            Authorization: `Bearer ${user.token}`,
