  ...
  const getDocDetailsList = async () => {
    ...
    try {
      const { data } = await axios.get(
        `https://dentalhouse-admin.vimubds5.a2hosted.com//api/v1/admin/getEmployeeDataByBranch/${user.branch_name}`,
        {
          headers: {
  ...
  const downloadEmployeeData = async () => {
    try {
      const { data } = await axios.post(
        `https://dentalhouse-admin.vimubds5.a2hosted.com//api/v1/admin/downloadStaffReport/${user.branch_name}`,
        {},
        {
          headers: {
            "Content-Type": "application/json",
