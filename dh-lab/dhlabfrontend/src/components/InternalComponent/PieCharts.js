...
//       try {
//         const response = await axios.get('https://dentalhouse.lab.vimubds5.a2hosted.com/api/lab/get-patient-details', {
//           headers: {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          const filteredData = response.data.result.filter(
            (dataset) =>
              ["oral", "pathology", "radiology"].includes(
                dataset.lab_name.toLowerCase()
              ) && dataset.test_status.toLowerCase() === "pending"
          );
          ...
          // Convert labCounts object to an array of objects
          const pieChartData = Object.entries(labCounts).map(
            ([lab_name, count]) => ({ lab_name, count })
          );
              ...
              {data && data.length > 0 ? (
                <Pie
                  data={{
                    labels: data.map(
                      (entry) =>
                        entry.lab_name.charAt(0).toUpperCase() +
                        entry.lab_name.slice(1)
                    ),
                    datasets: [
                      {
                        label: "# Patients",
