...
const AppointTable = () => {
  ...
  const handleStatusChange = async (appointmentId, patient_uhid, newStatus) => {
    try {
      ...
      await axios.put(
        `https://dentalhouse-receptionist.vimubds5.a2hosted.com/api/v1/receptionist/update-appointment-status`,
        {
          status: newStatus,
  ...
  const handleStatusCancel = async (appointmentId, patient_uhid, newStatus) => {
    ...
    try {
      // Send a PUT request to your backend endpoint to update the status
      await axios.put(
        `https://dentalhouse-receptionist.vimubds5.a2hosted.com/api/v1/receptionist/update-appointment-status-cancel`,
        {
          status: newStatus,
          cancelReason: reason,
          appointmentId: appointmentId,
