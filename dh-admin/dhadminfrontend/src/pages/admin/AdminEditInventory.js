  ...
  const getInvListDetails = async () => {
    try {
      const { data } = await axios.get(
        `https://dentalhouse-admin.vimubds5.a2hosted.com//api/v1/admin/getPurchaseInvByPurId/${branch.name}/${pid}`,
        {
          headers: {
  ...
  const updatePurchaseDetails = async (e) => {
    ...
    try {
      const { data } = await axios.put(
        `https://dentalhouse-admin.vimubds5.a2hosted.com///api/v1/admin/updatePurInvoice/${branch.name}/${pid}`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
