...
const FinancialReportCard = () => {
  ...
  const getPurchaseList = async () => {
    try {
      const response = await axios.get(
        `https://dentalhouse-admin.vimubds5.a2hosted.com//api/v1/admin/downloadEarnReportByTime/${user.branch_name}`,
        {
          headers: {
            "Content-Type": "multipart/form-data",
            Authorization: `Bearer ${user.token}`,
  ...
  const downloadExpenseData = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        `https://dentalhouse-admin.vimubds5.a2hosted.com//api/v1/admin/downloadExpenseReportByTime/${user.branch_name}`,
