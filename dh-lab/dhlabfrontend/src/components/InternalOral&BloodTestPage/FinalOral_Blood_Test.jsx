...
function FinalOral_Blood_Test() {
  ...
  const fetchPatientLabReports = async () => {
    try {
      const response = await axios.get(
        `https://dentalhouse.lab.vimubds5.a2hosted.com/api/lab/get-patient-lab-reports/${id}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
    ...
    if (isConfirmed) {
      try {
        const response = await axios.delete(
          `https://dentalhouse.lab.vimubds5.a2hosted.com/api/lab/delete-patient-lab-report/${id}`,
          {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${token}`,
  ...
  const fetchPatientTestDetails = async () => {
    try {
      const response = await axios.get(
        `https://dentalhouse.lab.vimubds5.a2hosted.com/api/lab/get-patient-test-details-by-id/${id}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  useEffect(() => {
    const fetchNotes = async () => {
      try {
        const response = await axios.get(
          `https://dentalhouse.lab.vimubds5.a2hosted.com/api/lab/getpatienttest-notes/${id}`,
          {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${token}`,
  ...
  const handleDelete = async (id) => {
    ...
    if (isConfirmed) {
      try {
        const response = await axios.delete(
          `https://dentalhouse.lab.vimubds5.a2hosted.com/api/lab/patent-details/${id}`,
          {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${token}`,
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.put(
        `https://dentalhouse.lab.vimubds5.a2hosted.com/api/lab/update-patent-test/${id}`,
        {
  ...
  const formattedAdjustedCollection_Date =
    parsedCollection_Date.format("DD-MM-YYYY");

  const parsedAuthenticate_Date = moment(patientauthenticate_date);
  const formattedAdjustedAuthenticate_Date =
    parsedAuthenticate_Date.format("DD-MM-YYYY");
  ...
  const handleViewPDF = (filePath) => {
    ...
    if (!newTab) {
      alert(
        "Failed to open PDF or other file in a new tab. Please check popup blocker settings."
      );
    }
  };
      ...
      <div clasNameName="main">
        <div className="container-fluid">
          <div className="row flex-nowrap ">
            ...
            <div
              className="col-xxl-11 col-xl-11 col-lg-11 col-md-12 col-sm-12 p-0 resp "
                              ...
                              {filetests.map((filetest, index) => (
                                <div key={index}>
                                  <Button
                                    variant="primary"
                                    onClick={() =>
                                      handleViewPDF(filetest.file_path)
                                    }
                                    className="mx-2 mt-2"
                                  >
                                  ...
                                  <Button
                                    variant="danger"
                                    onClick={() =>
                                      handleDeleteLabReport(filetest.id)
                                    } // Assuming each filetest object has an id
                                    className="mx-2 mt-2"
                                  >
                                    Delete
                                  </Button>
