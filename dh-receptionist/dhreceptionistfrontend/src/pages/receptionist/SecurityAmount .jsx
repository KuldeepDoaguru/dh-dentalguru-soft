...
function SecurityAmount() {
  ...
  const getSecurityAmountList = async () => {
    ...
    try {
      const { data } = await axios.get(
        `https://dentalhouse-receptionist.vimubds5.a2hosted.com/api/v1/receptionist/getSecurityAmountDataByBranch/${branch}`,
        {
          headers: {
  ...
  const MakeRefund = async (e) => {
    ...
    try {
      setLoading(true);
      const response = await axios.put(
        `https://dentalhouse-receptionist.vimubds5.a2hosted.com/api/v1/receptionist/updateRefundAmount/${selected}`,
        {
  ...
  const paySecurityCash = async (e) => {
    ...
    try {
      setLoading(true);
      const response = await axios.put(
        `https://dentalhouse-receptionist.vimubds5.a2hosted.com/api/v1/receptionist/updatePatientSecurityAmt/${selected}`,
        updatedData,
        {
          headers: {
            "Content-Type": "application/json",
  ...
  //     try {
  //       const { data } = await axios.get(
  //         `https://dentalhouse-receptionist.vimubds5.a2hosted.com/api/v1/receptionist/getSecurityAmountDataBySID/${id}`
  //       );
  //       console.log(data);
  //       setOutStanding(data);
  //     } catch (error) {
  ...
  //     try{
  //       const response = await axios.get(`https://dentalhouse-receptionist.vimubds5.a2hosted.com/api/v1/receptionist/patient-securityAmt/${branch}`);
  //       console.log(response);
  ...
  const formDetails = {
    ...
    message:
      `Dear ${filterForSecAmountDef[0]?.patient_name}, UHID ${filterForSecAmountDef[0]?.uhid} you have successfully paid ${filterForSecAmountDef[0]?.amount}/- as security amount.\n` +
      `Clinic Details:\n` +
  ...
  const sendWhatsappTextOnly = async () => {
    try {
      const res = await axios.post(
        "https://dentalhouse-receptionist.vimubds5.a2hosted.com/api/v1/receptionist/sendWhatsapptextonly",
        formDetails,
        {
          headers: {
            "Content-Type": "application/json",
  ...
  const refundformDetails = {
    phoneNumber: filterForSecAmountDef[0]?.patient_number,
    message:
      `Dear ${filterForSecAmountDef[0]?.patient_name}, UHID ${filterForSecAmountDef[0]?.uhid} you have successfully Refunded ${filterForSecAmountDef[0]?.remaining_amount}/- from security amount.\n` +
      `Clinic Details:\n` +
      `Name: ${currentBranch[0]?.hospital_name}\n` +
      `Contact: ${currentBranch[0]?.branch_contact}\n` +
      `Address: ${currentBranch[0]?.branch_address}\n` +
