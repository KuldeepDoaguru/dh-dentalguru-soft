  ...
  const updateLabData = async (e) => {
    ...
    try {
      const response = await axios.put(
        `https://dentalhouse-admin.vimubds5.a2hosted.com//api/v1/admin/updateLabDetails/${selectedItem.lab_id}`,
        upLabField,
        {
          headers: {
            "Content-Type": "application/json",
    ...
    try {
      ...
      if (isConfirmed) {
        const response = await axios.delete(
          `https://dentalhouse-admin.vimubds5.a2hosted.com//api/v1/admin/labDelete/${id}`,
          {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${user.token}`,
